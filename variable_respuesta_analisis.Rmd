...
# Seleccionar solo SalePrice y las variables numéricas
...
# Extraer la fila de SalePrice y convertirla en un dataframe
correlacion_saleprice <- as.data.frame(correlaciones["SalePrice", ], stringsAsFactors = FALSE)

# Convertir nombres de filas en una columna del dataframe
correlacion_saleprice$Variable <- row.names(correlacion_saleprice)

# Asegurarse de que los nombres de las filas no son necesarios como índice
row.names(correlacion_saleprice) <- NULL

# Renombrar la columna de correlación para mayor claridad
colnames(correlacion_saleprice)[1] <- "Correlacion"

# Ordenar por valor absoluto de la correlación para resaltar las más fuertes
correlacion_saleprice <- correlacion_saleprice %>%
  arrange(desc(abs(Correlacion)))

# Usar kable para dar estilo a la tabla
...
# Lista de variables cualitativas a analizar
...
# Función para realizar ANOVA y extraer el tamaño del efecto
...
# Calcular el valor p, Eta Squared, y Omega Squared para cada variable cualitativa
