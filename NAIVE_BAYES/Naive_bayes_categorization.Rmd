...
# Definir el control de la validación cruzada (k-fold, por ejemplo k=10)
```{r preparation_data}
ctrl <- trainControl(method = "cv",  # Método de validación cruzada
                     number = 5,    # Número de folds (k=10)
                     classProbs = TRUE,  # Para obtener probabilidades
                     summaryFunction = multiClassSummary)  # Métricas para clasificación multiclase


#Manejo de  NAN
na_columns_train <- colnames(train_data)[colSums(is.na(train_data)) > 0]
print("Columnas con NA en train_data:")
print(na_columns_train)

na_columns_test <- colnames(test_data)[colSums(is.na(test_data)) > 0]
...
impute_mode <- function(x) {
  if (is.factor(x)) {
    ...
    ifelse(is.na(x), mode_value, as.character(x))
  } else {
    x
  }
}
train_data <- train_data %>% mutate(across(where(is.factor), ~ impute_mode(.)))
test_data <- test_data %>% mutate(across(where(is.factor), ~ impute_mode(.)))
```

```{r}
# Entrenar el modelo con validación cruzada
set.seed(123)  # Fijar semilla para reproducibilidad
model_cv <- train(consumoCat ~ .,           # Fórmula del modelo
                  data = train_data,        # Datos de entrenamiento
                  method = "naive_bayes",   # Método: Naive Bayes
                  trControl = ctrl,         # Control de validación cruzada
                  metric = "Accuracy")      # Métrica a optimizar (precisión)
